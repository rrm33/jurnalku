...
import { getMapel, saveMapel, deleteMapel } from "@/actions/master";
...
export default function MapelPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await saveMapel(formData);
    if (res.success) {
    ...
    <div className="max-w-4xl mx-auto pb-10">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Daftar mata pelajaran yang diajarkan.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan="3" className="p-8 text-center text-slate-400 font-medium">Memuat data...</td></tr>
            ) : dataList.length === 0 ? (
              <tr><td colSpan="3" className="p-8 text-center text-slate-400 font-medium">Belum ada data.</td></tr>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
            <h3 className="text-lg font-bold text-slate-800 mb-4">{formData.id ? 'Edit' : 'Tambah'} Mata Pelajaran</h3>
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                <label className="block text-sm font-bold text-slate-600 mb-1">Nama Mata Pelajaran</label>
                <input 
                  type="text" 
                  required
                  value={formData.nama}
